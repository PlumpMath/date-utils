...
(defn parse-time-zone-only [^String time-zone]
  (check-pattern-condition :numbers+colons+Z time-zone)
  (if (=  time-zone "Z")
    "Z" ;; else we dont care about these signs in   formatting time zone value +/-
    (str (first time-zone) (parse-time (apply str (next time-zone))))))

(defn parse-time-only
...
(defn parse-date-time [^String s]
  (let [[date time] (str/split s #"T")
        date-parsed (parse-date-only date)
        time-parsed (parse-time-only (take-off-timezone time))
        time-zone (parse-time-zone-only (get-timezone time))]
    (format "%sT%s%s" date-parsed time-parsed time-zone)))
  ...
  (let [s (str/upper-case -s)]
    (if (substring? "T" s)
      (tf/parse (:basic-date-time-no-ms tf/formatters)
                (parse-date-time s))
      (tf/parse (:basic-date tf/formatters) (parse-date-only s)))))
